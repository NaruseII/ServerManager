...
public class ServerList {
    ...
    public static Server createNewServer(String name, int port, String host, int serverManagerPort, CoreServerType coreServerType){
        ...
        if(ServerManager.get().getCoreData().getCoreServerType().is(CoreServerType.PACKET_MANAGER, CoreServerType.BUNGEE_MANAGER, CoreServerType.VELOCITY_MANAGER)){
            GlobalLogger.info("Registering server '"+name+"' -> ["+server.getAddress().getHostAddress()+"] Port: "+port+" SMPort: "+serverManagerPort);
        }
        map.put(name, server);
    ...
    public static void deleteServer(String name) {
        ...
        if(server != null) {
            ...
            map.remove(name);
            if(ServerManager.get().getCoreData().getCoreServerType().is(CoreServerType.PACKET_MANAGER, CoreServerType.BUNGEE_MANAGER, CoreServerType.VELOCITY_MANAGER)) {
                GlobalLogger.info("Deleting server '" + name + "' -> ["+server.getAddress().getHostAddress()+"] Port: "+server.getPort()+" SMPort: "+server.getServerManagerPort());
            }
